...
import introspectionQueryResultData from 'decap-cms-backend-github/src/fragmentTypes';
import { APIError } from 'decap-cms-lib-util';
const NO_CACHE = 'no-cache';
const fragmentMatcher = new IntrospectionFragmentMatcher({
  introspectionQueryResultData
});
export default class API extends GraphQLAPI {
  constructor(config) {
    super({
    ...
    // Re-create Apollo Client with async token handling
  ...
  hasWriteAccess() {
    return super.hasWriteAccess().then(() => true).catch(error => {
      if (error.status === 401) {
