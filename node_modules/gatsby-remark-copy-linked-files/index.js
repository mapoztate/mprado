...
const newLinkURL = (linkNode, options, pathPrefix) => {
  ...
  return `${pathPrefix ? pathPrefix : ``}${startsWithSlash ? `` : `/`}${destination}`;
};
...
module.exports = ({
  ...
  visit(markdownAST, [`html`, `jsx`], node => {
    ...
    // Handle video tags.
    extractUrlAttributeAndElement($(`video source[src], video[src]`), `src`).forEach(processUrl);

    // Handle video poster.
    extractUrlAttributeAndElement($(`video[poster]`), `poster`).forEach(extractedUrlAttributeAndElement => processUrl({
      ...extractedUrlAttributeAndElement,
      isRequired: true
    }));

    // Handle audio tags.
    extractUrlAttributeAndElement($(`audio source[src], audio[src]`), `src`).forEach(processUrl);

    // Handle flash embed tags.
    extractUrlAttributeAndElement($(`object param[value]`), `value`).forEach(processUrl);
